...
/**    
 *     
 * 项目名称：cps-web-backend    
 * 类名称：BeanUtils    
 * 类描述：    
 * 创建人：wangjingcheng 
 * 创建时间：2016年5月13日 上午10:17:37    
 * 修改人：wangjingcheng   
 * 修改时间：2016年5月13日 上午10:17:37    
 * 修改备注：    
 * @version         
 */
public class BeanUtils {
     ...
     */
    public static void copy(Object dataObject, Object toObject)
    {
        Method[] methods = dataObject.getClass().getDeclaredMethods();
        for (Method m : methods)
        {
            if (m.getName().startsWith("get"))
            {
                ...
                {
                    FastMethod dafm =
                        FastClass.create(Thread.currentThread().getContextClassLoader(), dataObject.getClass())
                            .getMethod(m);
                    Method toMethod =
                        toObject.getClass().getDeclaredMethod(m.getName().replaceFirst("get", "set"),
                            new Class[] {m.getReturnType()});
                    FastMethod tofm =
                        FastClass.create(Thread.currentThread().getContextClassLoader(), toObject.getClass())
                            .getMethod(toMethod);
